...
def isValid(c, list, n):
	for c_x in list:
		if c_x != c:
			a_int = int(c, 16) ^ int (c_x, 16)
			if not ((a_int >= 65 and a_int <= 90) or (a_int >= 97 and a_int <= 122) or (a_int == 0)):
				return False
	return True
...
def main():
	global p11
	k = 0
	for i in range(0, len(c11), 2):
		j = i+1
		cs11[k] = int(c11[i]+c11[j], 16)
		cc_i = [c_m[i]+c_m[j] for c_m in cc]
		for n in range(10):
			if isValid(cc_i[n], cc_i, n):
				# use cc_i[n] to decrypt c11[n]
				c11_i = int(c11[i]+c11[j], 16)
				p11[k] =  chr(c11_i ^ int(cc_i[n], 16) ^ 32)
		k = k + 1

	print(p11)
	# get key from p11 xor c11
	k = 0
	for b in p11:
		key[k] = cs11[k] ^ ord(p11[k])
		k = k + 1
